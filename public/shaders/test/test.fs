...
vec3 illuminate()
{
   float coefficient = u_light.fade.x;
   float linear = u_light.fade.y;
   float quadratic = u_light.fade.z;
   ...
   return (ambient + diffuse + specular) * (1.0 / (coefficient + (linear * l) + (quadratic * pow(l, 2.0))));
}
